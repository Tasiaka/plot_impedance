...
class ImpedancePlotterApp(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Magnetic Impedance 3D Plotter (Объемный вид, ускоренный)")
        self.setGeometry(100, 100, 1200, 800)

        self.data = None
        self.cbar = None # атрибут colorbar
    ...
    def _create_widgets(self):
        ...
        self.grid_res_spinbox.setValue(150) # Можно уменьшить для ускорения
    ...
    def _update_plot(self):
        ...
        # Используем линейную интерполяцию для скорости, если нужно
        zi = griddata((x_data_filtered, y_data_filtered), z_magnitude_filtered, (xi, yi), method='linear') # или 'cubic'
        ...
        # --- УСКОРЕНИЕ ВРАЩЕНИЯ: Увеличиваем rstride/cstride ---
        plot_stride = 8 # Установить значение (например, 5, 8, 10) для ускорения вращения
        print(f"Plotting surfaces with stride={plot_stride}")

        # Рисуем ВЕРХНЮЮ поверхность
        surf_top = self.ax.plot_surface(xi, yi, zi_smoothed, cmap=cmap_name, antialiased=True,
                                        shade=True, alpha=1.0, norm=norm,
                                        rstride=plot_stride, cstride=plot_stride) # Используем stride
